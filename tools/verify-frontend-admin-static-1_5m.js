...
const appRequiredPatterns = [
  ...
  { name: "ton_deposit_pageshow_preserve", pattern: /PAGE_SHOW_PRESERVE_MODAL_IDS[\s\S]*tonDepositModal[\s\S]*handlePageshowModalState/ },
  { name: "ton_copy_clipboard_fallback", pattern: /clipboard_copy_failed[\s\S]*copy_address_fallback_select/ },
...
const adminRequiredPatterns = [
  ...
  { name: "admin_idempotency_header", pattern: /createIdempotencyKey[\s\S]*['"]Idempotency-Key['"]/ },
...
function checkConfig() {
  ...
  const context = {};
  try {
    vm.runInNewContext(config, { window: context }, { filename: "config.js" });
  } catch (error) {
  ...
  const allowedAssignments = [
    /^window\.VIDIPAY_API_BASE\s*=\s*"https:\/\/vidipay-backend-1\.onrender\.com";$/,
    /^window\.VIDIPAY_FRONTEND_BUILD\s*=\s*"[^"]+";$/
  ];
  const lines = config.split(/\r?\n/).map(line => line.trim()).filter(Boolean);
  const unexpected = lines.filter(line => !allowedAssignments.some(pattern => pattern.test(line)));
  if (unexpected.length) {
    fail(`config.js contains unexpected line(s): ${unexpected.join(" | ")}`);
  }
}
